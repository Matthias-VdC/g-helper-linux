...
public class LinuxNvidiaGpuControl : IGpuControl
{
    ...
    public (int defaultW, int minW, int maxW, int enforcedW)? GetPowerLimits()
    {
        ...
        {
            s = s.Trim();
            if (double.TryParse(s, CultureInfo.InvariantCulture, out double v)) return v;
            return -1;
        }
        ...
        if (def < 0 || min < 0 || max < 0) return null;
    ...
    public void ApplyGpuSettings(int powerW, int clockLockMhz)
    {
        ...
        Helpers.Logger.WriteLine($"NVIDIA: Applied power={powerW}W, clock={
            (clockLockMhz > 0 ? $"{clockLockMhz}MHz" : "unlocked")} (single pkexec)");
    }
